...
<nav class="mx-auto site-navigation">
  <div class="ml-auto">
    ...
    <a href="{{url('index')}}" class="btn btn-primary border-width-2 d-none d-lg-inline-block"><span
        ...
        <a href="{{url('reviewjob',['user_id' => $user->id])}}" class="btn btn-primary border-width-2 d-none d-lg-inline-block">Review Posted Jobs</a>
    ...
    <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Applied Applications</h1>
    ...
    <div class="col-md-6 col-lg-4 mb-5">
        <div class="border rounded p-3 shadow-sm bg-light">
            <h3><a class="text-black" href="#">{{$application->job->title}}</a></h3>
            <div>{{$application->job->company}}</div>
            <h3><a class="text-black" href="#">{{$application->name}}</a></h3> 
            <div>{{$application->email}} <span class="mx-2">|</span> <a>{{$application->gender}}</a></div>

            <div class="d-flex justify-content-between mt-3">
                <form action="{{ url('application/approve', ['application_id' => $application->id]) }}" method="POST">
                    @csrf
                    <button type="submit" class="btn btn-success btn-sm">Approve</button>
                </form>
                <form action="{{ url('application/decline', ['application_id' => $application->id]) }}" method="POST">
                    @csrf
